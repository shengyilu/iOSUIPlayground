...
open class PopTip: UIView {
    ...
    /// Holds the Color with the border of the arrow
    @objc open dynamic var arrowBorder = UIColor.clear
    /// The width for the poptip's border
    @objc open dynamic var arrowBorderWidth = CGFloat(2.0)
    ...
    open override func draw(_ rect: CGRect) {
        ...
        let background_path = PopTip.pathWith(rect: rect, frame: frame, direction: direction, arrowSize: arrowSize, arrowPosition: arrowPosition, borderWidth: borderWidth, radius: cornerRadius)
        
        arrowBorder.setFill()
        background_path.fill()
        
        arrowBorder.setStroke()
        background_path.lineWidth = borderWidth
        background_path.stroke()
        
        let path = PopTip.pathWith(rect: CGRect(origin: frame.origin, size: CGSize(width: frame.size.width, height: frame.size.height - 2)), frame: frame, direction: direction, arrowSize: arrowSize, arrowPosition: (direction == .up) ? CGPoint(x: arrowPosition.x, y: arrowPosition.y - 2) : arrowPosition, borderWidth: borderWidth, radius: cornerRadius)
